...
> :bug: This will compile, but if `?open=true` gives you back an empty array no matter what, check the type of `DateCompleted` on `ServiceTicket`. Comparing it to `null` only does what you expect if that property's type allows `null` values, the same fix you may have already made to `EmployeeId` back in `honeyrae-05`. If you haven't made that fix yet, go do it now.
