# Testing the Web API with Yaak

In this chapter you will learn how to test endpoints in an API using Yaak and how to add an endpoint to the API.

# Using Yaak

Yaak is a great tool for exploring APIs. You may already have it installed from the front end part of the course, but if not:

1. Visit [yaak.app](https://yaak.app/) and click the download button for your operating system.
1. Follow the installation instructions.
1. Launch the application.
1. Click the `+` button (or use the keyboard shortcut) to open a new request, and choose `HTTP`.
...
1. In the console, you will once again see the host addresses for the api while it is running. It is serving the `http` only option (this is the default when using `dotnet run`). For Yaak, use the `http` host and port. We are going to test the weather forecast endpoint. In `Program.cs` you can see the `url` that goes with that endpoint (`/weatherforecast`). So, in order to make a request to that endpoint, the full address will be `http://localhost:<port>/weatherforecast`. Paste this address into the URL field in Yaak.
1. Make sure the request method is set to `GET` (it should be the default), because the endpoint in `Program.cs` uses `MapGet`, not `MapPost` (there are others as well...)
...
1. Hover over the variable `forecast`. VS Code should show you the five forecast objects in an array that are about to be sent back to the client (in this case, Yaak). 
1. A small panel in VS Code will have a similar set of buttons to those in the browser dev tools that allow you to Step Over, In, Out, etc... Click the "Continue" button (or hit `F5` if you have the option to), and you will see that the program continues, and Yaak receives the response. 
...
Up Next: [Defining types for HoneyRaesAPI](./honeyrae-03-defining-types.md)