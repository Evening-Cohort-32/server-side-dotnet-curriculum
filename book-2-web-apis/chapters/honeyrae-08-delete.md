...
1. Test the endpoint in Yaak (don't forget to use the `DELETE` method and specify an `id` in the URL! Did you restart your server?).
...
1. You can also try the `GET` endpoint in Yaak to get the specific service ticket you deleted. That request should now get a `404` response. 

Up Next: [Assigning and Completing Tickets](./honeyrae-09-put.md)