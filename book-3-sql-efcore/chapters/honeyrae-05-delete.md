...
> :bulb: Every `Employee` endpoint now reads from and writes to the database instead of the `employees` collection at the top of `Program.cs`, so that collection itself doesn't do anything anymore, **unless** you have other endpoints that still read from it. If your `ServiceTicket` endpoints haven't been converted to SQL yet, they may still be looking up assigned employees out of that same `employees` collection, so leave it in place for now. Once every endpoint in your API, `Employee`, `Customer`, and `ServiceTicket` alike, is reading from and writing to the database, none of the collections you seeded `Program.cs` with back in Book 2 are doing anything anymore, and you're free to comment them out or delete them entirely.
