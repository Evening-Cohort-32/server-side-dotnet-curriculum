...
1. Start debugging the project, and test the endpoint. That's right! It should work with no other code. A few things to notice about the endpoint:
    ...
    - Check the Debug Console after testing this endpoint. That exact SQL query, the one just described above, should be logged there. This is the payoff of the habit you built of keeping an eye on the Debug Console back when you first started debugging web APIs. From now on, any time you write a Linq query against a `DbContext`, you can check the Debug Console to see the real SQL it produced. Comparing the two is a great way to build up your ability to read SQL, and to confirm that the query you wrote is actually doing what you think it's doing.
