...
> :bulb: This example is only demonstrating the `IsDBNull` technique, it's not the whole endpoint to paste in as-is. Your actual `/api/servicetickets` endpoint already has an `open` query string param and nested `Employees` data from Book 2, don't lose those when you rewrite it to hit the database. You'll bring the employees back the same way you will for the single-ticket endpoint below, this one just needs the same kind of `JOIN` through `ServiceTicketEmployee`, once for every ticket instead of one.
